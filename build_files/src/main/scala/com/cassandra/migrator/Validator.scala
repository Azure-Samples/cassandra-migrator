    ...
    val sourceSettings = config.source match {
      ...
      case otherwise =>
        throw new RuntimeException(
          s"Validation only supports validating against Cassandra wire protocol compatible databases " +
            s"(found ${otherwise.getClass.getSimpleName} settings)")
    }

    val targetSettings = config.target match {
      case s: TargetSettings.Cassandra => s
      case otherwise =>
        throw new RuntimeException(
          s"Validation only supports validating against Cassandra wire protocol compatible databases " +
            s"(found ${otherwise.getClass.getSimpleName} settings)")
